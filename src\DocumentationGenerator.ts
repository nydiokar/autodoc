...
export class DocumentationGenerator {
  ...
  private generateDefaultPRBody(): string {
    const summary = `This pull request includes updates to the JSDoc documentation in ${this.existingJsDocQueue.length} files across the repository. Changes were made to enhance code documentation and clarity.`;
    const fileList = this.existingJsDocQueue
      .map((item) => `- ${item.filePath.replace(/\\/g, '/')}`)
      .join('\n');

    const body = `${summary}\n\nModified files:\n${fileList}\n\nReviewers, please ensure the JSDoc updates adhere to best practices and enhance the overall documentation quality.`;
