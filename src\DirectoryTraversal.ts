...
/**
 * Class representing directory traversal functionality.
 * @class
 */
 ...
 * @param {Configuration} config - The configuration object for the instance.
 * @param {string[]} prFiles - An array of file paths for the instance, defaults to an empty array.
 */
  constructor(
  ...
  private isExcluded(absolutePath: string): boolean {
    ...
    const isExcludedDir = this.config.excludedDirectories.some((dir) =>
      relativeToTarget.split(path.sep).includes(dir),
    );
