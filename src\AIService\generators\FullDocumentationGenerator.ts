 ...
 */
/**
 * Interface representing a FAQ (Frequently Asked Question) object.
 * @typedef {Object} FAQ
 * @property {string} question - The question being asked.
 * @property {string} answer - The answer to the given question.
 */
interface FAQ {
  question: string;
  answer: string;
...
export class FullDocumentationGenerator {
  ...
  private async generateInstallation(packageJson: any): Promise<string> {
    ...
    try {
      ...
      const prompt = `Generate installation and integration instructions for this plugin:
            ...
            This is a plugin for the agent system. Generate comprehensive installation instructions that include:

            1. How to add the plugin to your project:
  ...
  private async generateBasicInstallPrompt(packageJson: any): Promise<string> {
    ...
    const prompt = `Generate basic installation instructions for this plugin:

        Plugin name: ${packageJson.name}
        Dependencies: ${JSON.stringify(packageJson.dependencies || {}, null, 2)}
        Peer dependencies: ${JSON.stringify(packageJson.peerDependencies || {}, null, 2)}

        This is a plugin for the agent system. Include basic setup instructions.`;
